...
from app.features.curriculum.schemas.curriculum import (
    CurriculumCreate,
    CurriculumUpdate,
    CurriculumResponse,
...
class CurriculumService(BaseService[Curriculum, CurriculumCreate, CurriculumUpdate]):
    ...
    def list_curricula(self, 
        ...
        if search_params and search_params.sort_by:
            sort_field = getattr(Curriculum, search_params.sort_by, Curriculum.display_order)
            sort_order_func = desc if search_params.sort_order == "desc" else asc
            query = query.order_by(sort_order_func(sort_field))
        else:
            query = query.order_by(asc(Curriculum.display_order))
    ...
    def duplicate_curriculum(self, 
        ...
        new_curriculum_data = {
            ...
            "display_order": 0  # Will be set to max + 1
        }
        
        # Set display order
        max_order = db.query(func.max(Curriculum.display_order)).filter(
            Curriculum.course_id == target_course_id
        ).scalar() or 0
        new_curriculum_data["display_order"] = max_order + 1
            ...
            try:
                ...
                if curriculum:
                    ...
                    if not request.preserve_order:
                        ...
                        curriculum.display_order = max_order + 1
