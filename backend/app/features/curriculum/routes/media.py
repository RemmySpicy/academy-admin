...
from app.features.curriculum.services.media_service import media_service
...
async def create_upload_session(
    ...
    try:
        upload_session = media_service.create_upload_session(
            db, upload_request, current_user["id"]
        )
...
async def upload_media_file(
    ...
    try:
        ...
        media = media_service.create_media(db, media_data, current_user["id"])
        return media
...
async def update_media(
    ...
    """
    try:
        media = media_service.update_media(db, media_id, media_data, current_user["id"])
        if not media:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Media not found"
...
async def get_media_usage(
    ...
    """
    try:
        usage = media_service.get_media_usage(db, media_id)
        if not usage:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Media not found"
...
async def get_processing_status(
    ...
    """
    try:
        status_info = media_service.get_processing_status(db, media_id)
        if not status_info:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Media not found"
