...
from app.features.courses.schemas.content_version import (
...
from app.features.courses.services.content_version_service import content_version_service
...
async def create_content_version(
    ...
    try:
        version = content_version_service.create_content_version(db, version_data, current_user["id"], program_context)
        return version
...
async def get_content_version_stats(
    ...
    """
    try:
        stats = content_version_service.get_content_version_stats(db, program_context)
        return stats
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_content_version(
    ...
    """
    version = content_version_service.get_content_version(db, version_id, program_context)
    if not version:
        raise HTTPException(
...
async def update_content_version(
    ...
    """
    try:
        version = content_version_service.update_content_version(db, version_id, version_data, current_user["id"], program_context)
        if not version:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Content version not found"
...
async def delete_content_version(
    ...
    """
    try:
        success = content_version_service.delete_content_version(db, version_id, program_context)
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Content version not found"
...
async def compare_content_versions(
    ...
    """
    try:
        comparison = content_version_service.compare_content_versions(db, version1_id, version2_id, program_context)
        if not comparison:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
...
async def get_current_version_for_lesson(
    ...
    """
    try:
        version = content_version_service.get_current_version_for_lesson(db, lesson_id, program_context)
        if not version:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
